...
:: Step 2: Parse cbuild-gen path from *.cbuild-gen-idx.yml and device-pack,
::         device name, pack path, and cgen.yml path from *.cbuild-gen.yml.
::         Derive design.modus path from the board directory (parent of tmp\).
set "PROJECT_NAME="
...
:: Parse device name from *.cbuild-gen.yml (field "  device: Vendor::DeviceName")
:: The "  device:" pattern avoids false matches on "device-pack:", "device-books:", etc.
for /f "delims=" %%a in ('findstr /C:"  device:" "!CBUILD_GEN_YML_PATH!"') do (
    set "DEVICE_NAME=%%a"
    set "DEVICE_NAME=!DEVICE_NAME:device:=!"
    set "DEVICE_NAME=!DEVICE_NAME: =!"
)
:: Strip vendor prefix (e.g. "Infineon::TLE9954EQA40" -> "TLE9954EQA40")
set "DEVICE_NAME_TMP="
for /f "tokens=2 delims=:" %%v in ("!DEVICE_NAME!") do set "DEVICE_NAME_TMP=%%v"
if defined DEVICE_NAME_TMP set "DEVICE_NAME=!DEVICE_NAME_TMP!"
echo DEBUG: DEVICE_NAME: !DEVICE_NAME! >> debug.log

if not defined DEVICE_NAME (
    echo Error: Could not parse device name from !CBUILD_GEN_YML_PATH!
    exit /b 1
)
...
:: Parse cgen.yml path from *.cbuild-gen.yml
...
:: Derive design.modus path from cbuild-gen.yml location: parent of tmp\ + config\design.modus
for %%i in ("!CBUILD_GEN_YML_PATH!") do set "TMP_DIR=%%~dpi"
set "TMP_DIR=!TMP_DIR:~0,-1!"
for %%i in ("!TMP_DIR!") do set "BOARD_DIR=%%~dpi"
set "BOARD_DIR=!BOARD_DIR:~0,-1!"
set "DESIGN_MODUS_SRC_PATH=!BOARD_DIR!\config\design.modus"
echo DEBUG: DESIGN_MODUS_SRC_PATH: !DESIGN_MODUS_SRC_PATH! >> debug.log

if not exist "!DESIGN_MODUS_SRC_PATH!" (
    echo Error: design.modus not found at !DESIGN_MODUS_SRC_PATH!
    exit /b 1
)
...
:: Step 3: Define DESIGN_PATH (fixed: board config\design.modus), GEN_DIR, YAML_FILE, and TMP_FILE
set "DESIGN_PATH=%DESIGN_MODUS_SRC_PATH%"
set "GEN_DIR=%CGEN_DIR%\GeneratedSource"
...
::         Looks for the root props.json (pack root) and device-info\device-db\props.json
set "LIBRARY_PATH="
set "ROOT_PROPS_FOUND=0"
...
:: Check for root props.json at the top level of the pack (e.g. TLE996x_7x_DFP\0.7.0\props.json)
if exist "!PACK_PATH!\props.json" (
    set "LIBRARY_PATH=!LIBRARY_PATH!!PACK_PATH!\props.json,"
    set "ROOT_PROPS_FOUND=1"
    echo Found root props.json: !PACK_PATH!\props.json >> debug.log
) else (
    set "MISSING_FILES=!MISSING_FILES!root props.json at !PACK_PATH!\props.json;"
    echo Missing root props.json: !PACK_PATH!\props.json >> debug.log
)
...
echo Device-db directory: !DEVICE_DB_PATH! >> debug.log

:: Check if any props.json files were found
if not defined LIBRARY_PATH (
    echo Error: No props.json files found in root pack or device-db directories
...
if "!ROOT_PROPS_FOUND!"=="0" (
    echo Warning: No root props.json found in pack
)
...
if not exist "%GEN_DIR%" (
    ...
    echo   - group: GeneratedSource >> "%TMP_FILE%"
    echo     files: >> "%TMP_FILE%"
) else (
    set "FILE_COUNT=0"
...
:: Step 9: Launch Device Configurator
::         --library : comma-separated props.json paths from pack (Step 5)
::         --design  : fixed path to board config\design.modus (Step 2)
::         --set-device : device name parsed from *.cbuild-gen.yml (Step 2)
echo Building Device Configurator command...
echo DEBUG: LIBRARY_PATH: !LIBRARY_PATH! >> debug.log
echo executed command --^> "%TOOL_PATH%" --library "%LIBRARY_PATH%" --design "%DESIGN_PATH%" --set-device "%DEVICE_NAME%"
start /B "" "%TOOL_PATH%" --library "%LIBRARY_PATH%" --design "%DESIGN_PATH%" --set-device "%DEVICE_NAME%"

:: Step 10: Wait for Device Configurator to start and then close, updating cgen.yml on design.modus saves
:: launch-tool.exe is only a launcher that exits immediately after spawning the real app.
:: Use PowerShell to detect any process whose executable path contains "InfineonDeviceConfigurator".
...
powershell -NoProfile -NonInteractive -Command "if (Get-Process | Where-Object { $_.Path -like '*InfineonDeviceConfigurator*' }) { exit 0 } else { exit 1 }" >nul 2>nul
if %ERRORLEVEL% equ 0 (
